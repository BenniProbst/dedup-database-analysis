...
The experimental testbed is a Kubernetes cluster consisting of four worker nodes. Each system under test is deployed as a separate workload (e.g., StatefulSet) with a dedicated PersistentVolumeClaim (PVC) of 50\,GiB, provisioned via a Longhorn StorageClass with a replica count of~4 (one replica per node). The exception is MinIO, which operates on direct disk without a Longhorn PVC (see Section~\ref{sec:measurement-infra} for details on its physical size measurement).

Longhorn implements distributed \emph{block storage} for Kubernetes by running an engine component and placing multiple volume \emph{replicas} on different nodes. In this testbed, a replica count $N=4$ is configured, meaning that \emph{every database's persistent storage} is synchronously replicated to all four cluster nodes at the block level. This design choice decouples storage-layer replication from application-layer replication: rather than relying on each database system's own replication protocol (e.g., PostgreSQL streaming replication, CockroachDB Raft consensus, or Kafka ISR), the Longhorn StorageClass uniformly provides four-way data redundancy beneath all systems. Each database therefore reads and writes a single logical volume, while Longhorn transparently maintains four physical copies. This uniform storage replication simplifies the experimental setup and ensures that the physical footprint measurement ($B_\text{phys}$) is comparable across all systems, since the replication factor is identical and externally controlled.
...
\begin{itemize}
    ...
    \item \textbf{comdare-DB} (very recent / experimental system; its internal storage metrics are accessed via a REST API endpoint and compared empirically with the other systems).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{CockroachDB} (resilient geo-distributed SQL database)~\cite{TaftEtAl2020CockroachDB}. CockroachDB shares its cluster with production workloads; it is provisioned with a 125\,GiB PVC to accommodate both production and lab data. Experiments are constrained to the \texttt{dedup\_lab} database and are limited to 50\,GiB of logical payload to avoid interfering with production data.
\end{itemize}
...
\paragraph{Database-internal instrumentation.}
Beyond external sampling, each system under test exposes native introspection interfaces that report storage and timing metrics at database-internal checkpoints. These are queried at stage boundaries (before/after each bulk or per-file operation) to obtain precise logical and physical size deltas:
\begin{itemize}[nosep]
    \item \textbf{PostgreSQL:} \texttt{pg\_total\_relation\_size()} reports the full on-disk footprint of a relation including TOAST and indexes. TOAST compression statistics (pglz/lz4) are observable via \texttt{pg\_statio\_all\_tables}. Per-query I/O timing is available through \texttt{EXPLAIN (ANALYZE, BUFFERS, TIMING)} and the \texttt{pg\_stat\_statements} extension (\texttt{blk\_read\_time}, \texttt{blk\_write\_time}).
    \item \textbf{MariaDB:} \texttt{INFORMATION\_SCHEMA.INNODB\_SYS\_TABLESPACES} provides both \texttt{FILE\_SIZE} (on-disk) and \texttt{ALLOCATED\_SIZE} (after page compression), yielding a direct InnoDB compression ratio. The \texttt{performance\_schema} records per-statement timing at picosecond resolution via \texttt{events\_stages\_history\_long}.
    \item \textbf{Redis:} \texttt{MEMORY STATS} decomposes memory into \texttt{dataset.bytes}, \texttt{overhead.total}, and per-database allocations. The \texttt{LATENCY} subsystem (\texttt{LATENCY LATEST}, \texttt{LATENCY HISTORY}) records per-event-class latency spikes, and \texttt{INFO commandstats} reports cumulative microsecond timing per command type.
    \item \textbf{Apache Kafka:} The JMX MBean \texttt{kafka.network:type=RequestMetrics} decomposes each produce request into five timing components: \texttt{RequestQueueTimeMs}, \texttt{LocalTimeMs}, \texttt{RemoteTimeMs}, \texttt{ResponseQueueTimeMs}, and \texttt{ResponseSendTimeMs}. Physical log sizes per partition are obtained via \texttt{AdminClient.describeLogDirs()}.
    \item \textbf{MinIO:} The built-in Prometheus endpoint exposes \texttt{minio\_s3\_ttfb\_seconds\_distribution} (time-to-first-byte histogram) for per-operation latency and \texttt{minio\_bucket\_usage\_total\_bytes} for logical bucket sizes. Erasure coding overhead is derived from the shard configuration.
    \item \textbf{ClickHouse:} The \texttt{system.columns} table reports both \texttt{data\_compressed\_bytes} and \texttt{data\_uncompressed\_bytes} per column, enabling direct per-column compression ratio analysis. \texttt{system.query\_log} records over 200 profile event counters per query, including \texttt{DiskReadElapsedMicroseconds} and \texttt{DiskWriteElapsedMicroseconds}. The \texttt{OPTIMIZE TABLE \ldots\ DEDUPLICATE} command for \texttt{ReplacingMergeTree} engines provides an explicit deduplication step whose effect is measurable via row-count deltas in \texttt{system.parts}.
    \item \textbf{CockroachDB:} \texttt{SHOW RANGES FROM TABLE \ldots\ WITH DETAILS} returns per-range \texttt{span\_stats} including \texttt{live\_bytes}, \texttt{approximate\_disk\_bytes}, and MVCC garbage metrics. The \texttt{crdb\_internal.statement\_statistics} view breaks SQL latency into parse, plan, run, service, and idle components.
    \item \textbf{comdare-DB:} An internal REST API endpoint exposes opaque storage metrics; the exact counters depend on the system's current development state and are recorded as-is for empirical comparison.
\end{itemize}
These database-internal measurements complement the Longhorn-level physical size observations: while Longhorn reports the block-storage footprint externally, the internal views reveal whether the database itself performs compression, deduplication, or space reclamation \emph{before} blocks are written to the volume. Discrepancies between internal ``bytes on disk'' and Longhorn \texttt{actual\_size\_bytes} indicate storage-layer overhead or, conversely, filesystem-level optimization invisible to the database.
